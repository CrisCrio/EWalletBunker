...
// ── 1. MODAL PIGGY BANK (METAS DE AHORRO) ───────────────────────────
...
// ── 4. NUEVO MODAL: PUNTOS ADSO ──────────────────────────────────────
function PuntosADSOModal({ visible, puntosActuales, onClose, onRedeem }) {
  ...
  const equivalenciaCOP = useMemo(() => {
    const pts = parseFloat(puntosARedimir);
    if (!isNaN(pts) && pts > 0) {
      return pts * 10; // Cada punto equivale a $10 COP
    }
    return 0;
  }, [puntosARedimir]);

  const handleRedimir = useCallback(() => {
    const pts = parseFloat(puntosARedimir);
    if (isNaN(pts) || pts <= 0) {
      setResultado({ error: 'Ingresa una cantidad válida' });
      return;
    }
    if (pts > puntosActuales) {
      setResultado({ error: 'No tienes suficientes puntos' });
      return;
    }

    const beneficioCOP = pts * 10;
    setResultado({ ok: true, cashback: beneficioCOP });
    onRedeem(pts, beneficioCOP);

    setTimeout(() => {
      handleClose();
    }, 1800);
  }, [puntosARedimir, puntosActuales, onRedeem, handleClose]);
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleClose}>
      <View style={modal.overlay}>
        <View style={modal.sheet}>
          <View style={modal.header}>
            <View style={[modal.usdtBadge, { backgroundColor: '#7c3aed' }]}>
              <Text style={[modal.usdtBadgeText, { color: '#ffffff' }]}>★</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={modal.title}>Mis Puntos ADSO</Text>
              <Text style={modal.subtitle}>Fidelización — Redime tus puntos por saldo</Text>
            </View>
            <TouchableOpacity onPress={handleClose} style={modal.closeBtn}>
              <Text style={modal.closeBtnText}>✕</Text>
            </TouchableOpacity>
          ...
          <View style={[modal.rateCard, { borderColor: '#ddd6fe', backgroundColor: '#faf5ff' }]}>
            <View style={{ flex: 1 }}>
              <Text style={[modal.rateLabel, { color: '#7c3aed' }]}>TASA DE REDENCIÓN</Text>
              <Text style={[modal.rateValue, { color: '#5b21b6' }]}>1 Punto ADSO = $10 COP</Text>
            </View>
            <View style={adsoStyles.badgePill}>
              <Text style={adsoStyles.badgePillText}>{puntosActuales.toLocaleString('es-CO')} Disponibles</Text>
            </View>
          </View>

          <View style={modal.inputGroup}>
            <Text style={modal.inputLabel}>¿Cuántos puntos deseas redimir?</Text>
            <View style={[modal.inputRow, { borderColor: '#e9d5ff' }]}>
              <Text style={[modal.currencyPrefix, { color: '#a78bfa' }]}>★</Text>
              <TextInput
                style={modal.input}
                value={puntosARedimir}
                onChangeText={(t) => { setPuntosARedimir(t); setResultado(null); }}
                placeholder="Ej: 500"
          ...
          {equivalenciaCOP > 0 && !resultado && (
            <View style={[modal.previewRow, { backgroundColor: '#f5f3ff' }]}>
              <Text style={[modal.previewLabel, { color: '#7c3aed' }]}>Recibirás como Cashback:</Text>
              <Text style={[modal.previewValue, { color: '#6d28d9' }]}>{formatCOP(equivalenciaCOP)}</Text>
            </View>
          )}

          {resultado?.error && (
            <View style={[modal.resultCard, modal.resultErr]}>
              <Text style={[modal.resultTitle, { color: '#dc2626' }]}>✗ Error</Text>
              <Text style={modal.resultDetail}>{resultado.error}</Text>
            </View>
          )}

          {resultado?.ok && (
            <View style={[modal.resultCard, { backgroundColor: '#f5f3ff', borderColor: '#ddd6fe', borderWidth: 1 }]}>
              <Text style={[modal.resultTitle, { color: '#6d28d9' }]}>✓ Redención exitosa</Text>
              <Text style={modal.resultDetail}>Has convertido tus puntos en dinero real.</Text>
              <Text style={[modal.resultDetail, { fontWeight: 'bold', color: '#7c3aed' }]}>+{formatCOP(resultado.cashback)} abonados a tu saldo.</Text>
            </View>
          )}

          <TouchableOpacity
            style={[adsoStyles.redeemBtn, (!puntosARedimir || resultado?.ok) && adsoStyles.redeemBtnDisabled]}
            onPress={handleRedimir}
            disabled={!puntosARedimir || !!resultado?.ok}
          >
            <Text style={adsoStyles.redeemBtnText}>Redimir Puntos</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
...
export default function WalletScreen() {
  ...
  const [showPuntosModal, setShowPuntosModal] = useState(false); // Estado para el nuevo modal
  const [usdtBalance, setUsdtBalance] = useState(0);
  const [puntosRedimidosDinamicos, setPuntosRedimidosDinamicos] = useState(0); // Control local de redención

  const totalBalance = useMemo(() => calculateNetBalance(allTransactions), [allTransactions]);
  
  // Restamos los puntos redimidos dinámicamente del total acumulado por historial
  const puntosADSO = useMemo(() => {
    const base = calcularPuntosADSO(allTransactions);
    const restante = base - puntosRedimidosDinamicos;
    return restante < 0 ? 0 : parseFloat(restante.toFixed(2));
  }, [allTransactions, puntosRedimidosDinamicos]);
  ...
  // Callback ejecutado tras redimir puntos en el modal
  const handlePuntosRedeem = useCallback((ptsRedimidos, beneficioCOP) => {
    setPuntosRedimidosDinamicos(prev => prev + ptsRedimidos);
    
    // Agrega un ingreso al balance general por concepto de cashback
    const cashbackTx = {
      id: `adso-redeem-${Date.now()}`,
      accountNumber: 'ADSO-REDEEM',
      type: 'Ingreso',
      amount: beneficioCOP,
      date: new Date(),
      status: 'Completado',
      nota: `Cashback: Redención de ${ptsRedimidos} pts`,
      puntosADSO: 0,
    };
    setAllTransactions(prev => [cashbackTx, ...prev]);
  }, []);
  ...
  const renderItem = ({ item }) => {
    ...
    const isAdsoRedeem = item.accountNumber === 'ADSO-REDEEM';
    
    return (
      <View style={[
        styles.card, 
        isUSDT && styles.usdtCard, 
        isPiggy && styles.piggyCard,
        isAdsoRedeem && adsoStyles.adsoCard
      ]}>
        <View style={styles.cardRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            {isUSDT && <View style={styles.usdtDot} />}
            {isPiggy && <View style={styles.piggyDot} />}
            {isAdsoRedeem && <View style={adsoStyles.adsoDot} />}
            <Text style={styles.accountText}>
              {isUSDT ? '₮ USDT' : isPiggy ? '🐷 Ahorro' : isAdsoRedeem ? '★ Puntos ADSO' : `Ref: ${item.accountNumber}`}
            </Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.balanceContainer}>
        ...
        {/* Contenedor horizontal para las etiquetas de balances adicionales */}
        <View style={styles.pillsRowContainer}>
          ...
          {puntosADSO > 0 && (
            <View style={[styles.adsoPill, { marginTop: 0 }]}>
        ...
        {/* Action Row optimizada para 4 botones fluidos */}
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity style={adsoStyles.actionBtn} onPress={() => setShowPuntosModal(true)} activeOpacity={0.85}>
            <Text style={adsoStyles.actionBtnText}>★ Puntos</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <PuntosADSOModal visible={showPuntosModal} puntosActuales={puntosADSO} onClose={() => setShowPuntosModal(false)} onRedeem={handlePuntosRedeem} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  balanceContainer: { backgroundColor: '#0f172a', padding: 20, margin: 16, borderRadius: 20, alignItems: 'center' },
  ...
  pillsRowContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginTop: 12 },
  usdtPill: { backgroundColor: '#1c3a2a', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: '#26d97f55' },
  usdtPillText: { color: '#26d97f', fontWeight: '700', fontSize: 12 },
  adsoPill: { backgroundColor: '#2d1a6e', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: '#7c3aed55' },
  adsoPillText: { color: '#a78bfa', fontWeight: '700', fontSize: 12 },
  actionRow: { flexDirection: 'row', gap: 6, marginTop: 18, width: '100%' },
  recargarBtn: { flex: 1, backgroundColor: '#3b82f6', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  recargarBtnText: { color: '#ffffff', fontWeight: '800', fontSize: 12 },
  buyUSDTBtn: { flex: 1, backgroundColor: '#26d97f', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  buyUSDTBtnText: { color: '#0f172a', fontWeight: '800', fontSize: 12 },
  piggyBtn: { flex: 1, backgroundColor: '#f59e0b', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  piggyBtnText: { color: '#0f172a', fontWeight: '800', fontSize: 12 },
...
// ── ESTILOS EXCLUSIVOS PARA EL CUARTO PUNTO (ADSO) ──────────────────
const adsoStyles = StyleSheet.create({
  actionBtn: { flex: 1, backgroundColor: '#7c3aed', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
  actionBtnText: { color: '#ffffff', fontWeight: '800', fontSize: 12 },
  badgePill: { backgroundColor: '#7c3aed', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4 },
  badgePillText: { color: '#ffffff', fontSize: 11, fontWeight: '700' },
  redeemBtn: { backgroundColor: '#7c3aed', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 12 },
  redeemBtnDisabled: { backgroundColor: '#ddd6fe' },
  redeemBtnText: { color: '#ffffff', fontWeight: '800', fontSize: 17 },
  adsoCard: { backgroundColor: '#faf5ff', borderLeftWidth: 3, borderLeftColor: '#7c3aed' },
  adsoDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#7c3aed' },
});